...
namespace BrightstarDB.Storage.BTree
{
    internal class InternalNode : IInternalNode
    {
        ...
        private InternalNode(PageStruct page,  BPlusTreeConfiguration treeConfig)
        {
            _page = page;
            _config = treeConfig;
            _keyCount = 0;
        ...
        public InternalNode(PageStruct page, int keyCount, BPlusTreeConfiguration treeConfig)
        {
            _config = treeConfig;
            _page = page;
            _keyCount = keyCount;
        ...
        public InternalNode(PageStruct page, byte[] splitKey, ulong leftPageId, ulong rightPageId,
        ...
        /// <param name="treeConfiguration">The tree configuration</param>
        /// <exception cref="InvalidOperationException">Raised if <paramref name="page"/> is not a writeable page</exception>
        public InternalNode(PageStruct page, ulong onlyChild, BPlusTreeConfiguration treeConfiguration)
        {
            _config = treeConfiguration;
            _page = page;
        ...
        /// <param name="treeConfiguration">The tree configuration</param>
        public InternalNode(PageStruct page, List<byte[]> keys, List<ulong> values, BPlusTreeConfiguration treeConfiguration)
        {
            _config = treeConfiguration;
            _page = page;
            KeyCount = keys.Count;
        ...
        public void DumpStructure(BPlusTree tree, int indentLevel)
        {
            ...
            for (int i = 0; i < _keyCount; i++)
            {
                ...
                childNode = tree.GetNode(childPointer);
            ...
            childNode = tree.GetNode(lastPointer);
            childNode.DumpStructure(tree, indentLevel + 1);
        }
        ...
        public bool Merge(WriteablePageStoreSession session, INode s, byte[] joinKey)
        {
        ...
        public IInternalNode Split(WriteablePageStoreSession session, PageStruct rightNodePage, out byte[] splitKey)
        {
#if DEBUG_BTREE
        ...
        public void Insert(WriteablePageStoreSession session, byte[] key, ulong childPointer)
        {
#if DEBUG_BTREE
        ...
        public void UpdateChildPointer(WriteablePageStoreSession session, ulong oldChildPointer, ulong newChildPointer)
        {
#if DEBUG_BTREE
        ...
        public bool RedistributeFromLeft(WriteablePageStoreSession session, IInternalNode leftSibling, byte[] joinKey, byte[] newJoinKey)
        {
        ...
        public bool RedistributeFromRight(WriteablePageStoreSession session, IInternalNode rightSibling, byte[] joinKey, byte[] newJoinKey)
        {
        ...
        public void SetLeftKey(WriteablePageStoreSession session, ulong childNodeId, byte[] childNodeKey)
        {
        ...
        public void SetKey(WriteablePageStoreSession session, ulong childNodeId, byte[] newKey)
        {
            var ix = Search(childNodeId);
        ...
        public byte[] RemoveChildPointer(WriteablePageStoreSession session, ulong childNodeId)
        {
        ...
        private void EnsureWriteable(WriteablePageStoreSession session)
        {
            if (!_page.IsWriteable)
            {
                _page = session.CopyPage(_page.PageNumber);
            }
        }
    ...
    {
        public DuplicateKeyException(ulong pageId)
        {
            throw new NotImplementedException();
        }
    }

    internal class NodeFullException : Exception
    {
    }
}
